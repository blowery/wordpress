...
var _19={l:0,r:0,t:0,b:0},s,_1b,a,b,c,d,gtb=dojox.gfx._base._getTextBox,gl=dc.scaler.common.getNumericLabel,_22=0,ta=this.chart.theme.axis,_24="font" in this.opt?this.opt.font:ta.font,_25="majorTick" in this.opt?this.opt.majorTick:ta.majorTick,_26="minorTick" in this.opt?this.opt.minorTick:ta.minorTick,_27=_24?g.normalizedLength(g.splitFontString(_24).size):0;
if(this.vertical){
if(_27){
s=this.scaler;
if(this.labels){
_1b=df.foldl(df.map(this.labels,function(_28){
return dojox.gfx._base._getTextBox(_28.text,{font:_24}).w;
}),"Math.max(a, b)",0);
}else{
a=gtb(gl(s.major.start,s.major.prec,this.opt),{font:_24}).w;
b=gtb(gl(s.major.start+s.major.count*s.major.tick,s.major.prec,this.opt),{font:_24}).w;
c=gtb(gl(s.minor.start,s.minor.prec,this.opt),{font:_24}).w;
d=gtb(gl(s.minor.start+s.minor.count*s.minor.tick,s.minor.prec,this.opt),{font:_24}).w;
_1b=Math.max(a,b,c,d);
}
_22=_1b+_6;
}
_22+=_6+Math.max(_25.length,_26.length);
_19[this.opt.leftBottom?"l":"r"]=_22;
_19.t=_19.b=_27/2;
}else{
if(_27){
_22=_27+_6;
...
_22+=_6+Math.max(_25.length,_26.length);
_19[this.opt.leftBottom?"b":"t"]=_22;
if(_27){
s=this.scaler;
if(this.labels){
_1b=df.foldl(df.map(this.labels,function(_29){
...
dx=_2f.x*_35.length;
dy=_2f.y*_35.length;
_3d=c.minMinorStep<=c.minor.tick*c.scale;
